...
<div align="center">
  <p><font size="4" color=black><b>小數加法測驗卷</b></font></p>
...
if($t1>=5){          //出題條件的判斷
  echo "<font color=red>出題條件錯誤！本系統最多只練習到4位數字的加法(含小數)......","<br><br>","請按上一頁重新出題......</font>";
};
...
for($i=1; $i<=20; $i++){    //此迴圈產生被加數與加數
  $add1[$i-1] = (rand(pow(10,$t1-1),pow(10,$t1)-1)) / pow(10,$add12);         //亂數產生被加數   
  $add2[$i-1] = (rand(pow(10,$t1-1),pow(10,$t1)-1)) / pow(10,$add12);         //亂數產生加數1  
  $add3[$i-1] = (rand(pow(10,$t1-1),pow(10,$t1)-1)) / pow(10,$add12);         //亂數產生加數2 
  $add4[$i-1] = (rand(pow(10,$t1-1),pow(10,$t1)-1)) / pow(10,$add12);         //亂數產生加數3 
};
echo "</font>";
...
for($j=1; $j<=20; $j++){    //此迴圈出考卷
  ...
  if($t2>=2 && $t2<=4){
    ...
    if($t2-2 == 0){ 
      echo $add1[$j-1],"＋",$add2[$j-1],"＝<br><br><br><br><br><br><br><br></td>";
      $sum[$j-1] = $add1[$j-1] + $add2[$j-1];       //計算2個數的和
    }elseif($t2-2 == 1){
      echo $add1[$j-1],"＋",$add2[$j-1],"＋",$add3[$j-1],"＝<br><br><br><br><br><br><br><br></td>";
      $sum[$j-1] = $add1[$j-1] + $add2[$j-1]+ $add3[$j-1];       //計算3個數的和
    }else{
      echo $add1[$j-1],"＋",$add2[$j-1],"＋",$add3[$j-1],"＋",$add4[$j-1],"＝<br><br><br><br><br><br><br><br></td>";
